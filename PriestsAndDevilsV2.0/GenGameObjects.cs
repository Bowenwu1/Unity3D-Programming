...
public class GenGameObjects : MonoBehaviour, ISceneController, IUserAction {
	...
	void Start () {
		actionManager = (CCActionManager)this.GetComponent("CCActionManager");
	}
	...
	void Update () {
		...
		if (game_state == GenGameObjects.State.normal)
			check ();
	}
	...
	void get_on_boat(GameObject temp) {
		game_state = GenGameObjects.State.moving;
		if (ship.transform.position == ship_begin_position) {
			if (onTheWay.Count == 1) {
				actionManager.RunAction (temp, actionManager.get_on_boat_left_begin, actionManager);
			} else {
				actionManager.RunAction (temp, actionManager.get_on_boat_right_begin, actionManager);				
			}
		} else {
			if (onTheWay.Count == 1) {
				actionManager.RunAction (temp, actionManager.get_on_boat_left_end, actionManager);
			} else {
				actionManager.RunAction (temp, actionManager.get_on_boat_right_end, actionManager);				
			}
		}
	}
	// get on the boat
	public void priest_to_boat_at_begin() {
		if (priest_begin.Count > 0 && onTheWay.Count < 2 && ship.transform.position == ship_begin_position) {
	...
	public void devil_to_boat_at_begin() {
		if (devil_begin.Count > 0 && onTheWay.Count < 2 && ship.transform.position == ship_begin_position) {
	...
	public void priest_to_boat_at_end() {
		if (priest_end.Count > 0 && onTheWay.Count < 2 && ship.transform.position == ship_end_position) {
	...
	public void devil_to_boat_at_end() {
		if (devil_end.Count > 0 && onTheWay.Count < 2 && ship.transform.position == ship_end_position) {
	...
	public void drive() {
		if (onTheWay.Count > 0) {
			if (ship.transform.position == ship_begin_position) {
				game_state = GenGameObjects.State.moving;
				actionManager.RunAction (ship, actionManager.boat_to_end, actionManager);
			} else if (ship.transform.position == ship_end_position) {
				game_state = GenGameObjects.State.moving;
				actionManager.RunAction (ship, actionManager.boat_to_begin, actionManager);
			}
		}
	}
